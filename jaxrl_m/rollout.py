...
def rollout_policy(agent,env,exploration_rng,
                   ...
                   discount,max_steps=None,max_rollouts=None,
                   eval=False,replay_buffer=None,actor_buffer=None):
    
    # Ensure at least one stopping condition is provided
    if max_steps is None and max_rollouts is None:
        raise ValueError("Either max_steps or max_rollouts must be provided")
    ...
    # Continue while both conditions are not met (if they exist)
    while ((max_steps is None or n_steps < max_steps) and 
           (max_rollouts is None or n_rollouts < max_rollouts)):
