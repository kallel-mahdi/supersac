...
parser.add_argument('--project_name',type=str,default="single_exp_off2") 
...
parser.add_argument('--full_off_policy',type=str2bool,default=True)
...
def train(args):
    ...
    if args.full_off_policy:
        actor_buffer = ReplayBuffer.create(example_transition, size=int(args.buffer_size))
    else:
    ...
    with tqdm.tqdm(total=max_steps) as pbar:
        ...
        while (i < max_steps):
                ...
                @jax.jit
                def compute_log_probs(actor, observations, pre_actions):
                ...
                actor_batch["log_probs"] = compute_log_probs(agent.actor, actor_batch["observations"], actor_batch["pre_actions"])
