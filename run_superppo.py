...
parser.add_argument('--project_name',type=str,default="SuperPPO") 
...
class SACAgent(flax.struct.PyTreeNode):
    ...
    def update_critics(agent,batch: Batch):
        ...
        def update_one_critic(critic):
                ...
                def critic_loss_fn(critic_params):
                        ...
                        p_actions,p_log_probs = next_dist.sample_and_log_prob(seed=next_key)
                        next_log_probs = p_log_probs - jnp.sum(2 * (jnp.log(2) - p_actions - jax.nn.softplus(-2 * p_actions)), axis=-1)
                        next_actions = jnp.tanh(p_actions)
    ...
    def update_actor(agent, batch: Batch,R2):
        ...
        def actor_loss_fn(
            ...
            p_actions = batch["p_actions"]
            p_log_probs = dist.log_prob(p_actions)
            new_logp = p_log_probs - jnp.sum(2 * (jnp.log(2) - p_actions - jax.nn.softplus(-2 * p_actions)), axis=-1)
        ...
        q = q.reshape(10,-1) ###HOTFIX : could cause problems better vmap
        v = jnp.mean(q,axis=0)
        h = -jnp.mean(log_p.reshape(10,-1),axis=0) ###HOTFIX
        
       
        
        
        ### Compute advantage for the fixed states AND actions
        q_all = jax.vmap(call_one_critic,in_axes=(None,None,0))(batch["observations"], batch["actions"],agent.critic.params)##critic_update_info
        q = jnp.mean(q_all,axis=0)
    ...
    def sample_actions(agent,   
        ...
        ### random always true
        distrib = agent.actor(observations, temperature=temperature)
        actions,log_probs = distrib.sample_and_log_prob(seed=seed)
...
def train(args):
    ...
    example_transition = dict(
        ...
        discounts=1.0,
        log_probs=0.,
        p_actions= env.action_space.sample()
    )
